...
``` r
drop_high_na_cols(data, threshold = 99, quiet = FALSE)
```
...
- threshold:
  ...
  Numeric in `[0, 100]`. Columns with `percent_na >= threshold` are
  removed. Default `99`.
...
`data` with high-NA columns removed.

## See also

[`summarize_na()`](https://anaboeriu14.github.io/adRutils/reference/summarize_na.md)
for inspecting NA patterns without removing columns.
